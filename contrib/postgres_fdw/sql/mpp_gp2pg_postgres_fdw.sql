...
-- It's unsafe to push down DISTINCT within aggregates for mpp_execute = 'multi servers'.
EXPLAIN (VERBOSE, COSTS OFF)
...
-- limit is not pushed down when mpp_execute is set to 'multi servers'
...
SELECT c1, c2 FROM mpp_ft2 order by c1 limit 3;
-- join is not safe to pushed down when mpp_execute is set to 'multi servers'
EXPLAIN (VERBOSE, COSTS OFF)
