...
postgresBeginForeignScan(ForeignScanState *node, int eflags)
{
	...
	if(Gp_role == GP_ROLE_EXECUTE && table->exec_location == FTEXECLOCATION_ALL_SEGMENTS)
	{
...
postgresIsForeignRelUpdatable(Relation rel)
{
	...
	/*
	 * It's unsafe to INSERT, UPDATE and DELETE when mpp_execute is 'all segments'.
	 */
...
postgresBeginDirectModify(ForeignScanState *node, int eflags)
{
	...
	if((Gp_role == GP_ROLE_DISPATCH && table->exec_location != FTEXECLOCATION_ALL_SEGMENTS) ||
	    Gp_role == GP_ROLE_EXECUTE)
		dmstate->conn = GetConnection(server, user, false, true);
						...
						Cost *p_startup_cost, Cost *p_total_cost)
{
	...
	if (fpinfo->use_remote_estimate)
	{
		...
		if(foreignrel->exec_location == FTEXECLOCATION_ALL_SEGMENTS)
		{
			...
			{
				rewrite_server_options(server, index);
				conn = GetConnection(server, fpinfo->user, false, false);
		...
		{
			conn = GetConnection(server, fpinfo->user, false, true);
					  ...
					  List *retrieved_attrs)
{
	...
	if((Gp_role == GP_ROLE_DISPATCH && table->exec_location != FTEXECLOCATION_ALL_SEGMENTS) ||
	    Gp_role == GP_ROLE_EXECUTE)
		fmstate->conn = GetConnection(server, user, false, true);
